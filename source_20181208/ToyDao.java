...
public class ToyDao {
	public static int save(String ToyName, String Type, String Age, String Company) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("insert into toylist(ToyName,Type,Age,Company) values(?,?,?,?)");
	...
	public static int SaveToy(String ToyN, String TypeN, String AgeN, String CompanyN) {
		int status = 0;
		try (Connection con = DB.getConnection()) {
			PreparedStatement ps = con
					.prepareStatement("insert into toylist(ToyName,Type,Age,Company) values(?,?,?,?)");
		    System.out.println(ToyN);
			ps.setString(1, ToyN);
	...
	public static int Delete(int ToyName) {
		int status = 0;
		try (Connection con = DB.getConnection()) {
			PreparedStatement ps = con.prepareStatement("DELETE FROM toylist where ToyName=?");
			ps.setInt(1, ToyName);
			status = ps.executeUpdate();
			con.close();
		} catch (Exception e) {
